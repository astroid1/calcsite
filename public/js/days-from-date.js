(function () {
  ...
  function parseDate(input) {
    ...
    if (m1) {
      const y = parseInt(m1[1], 10), mo = parseInt(m1[2], 10), da = parseInt(m1[3], 10);
      d = new Date(y, mo - 1, da);
      if (d && d.getFullYear() === y && d.getMonth() === mo - 1 && d.getDate() === da) return d;
      d = null;
    }
    const us = /^(\d{1,2})\/(\d{1,2})\/(\d{4})$/;
    const m2 = input.match(us);
    if (m2) {
      const mo = parseInt(m2[1], 10), da = parseInt(m2[2], 10), y = parseInt(m2[3], 10);
      d = new Date(y, mo - 1, da);
      if (d && d.getFullYear() === y && d.getMonth() === mo - 1 && d.getDate() === da) return d;
      d = null;
    }
  ...
  btn.addEventListener("click", () => {
    ...
    let verb;
    if (diff === 0) verb = "is today";
    else if (diff > 0) verb = "is in <strong>" + diff + "</strong> day" + plural(diff);
    else verb = "was <strong>" + Math.abs(diff) + "</strong> day" + plural(diff) + " ago";
